...
def merge_images_vertically(image1_path, image2_path, output_path, format='JPEG', quality=90):
    # Open the two images
    ...
    merged_image = Image.new(image1.mode, (max_width, 2 * max_height))

    # Paste the first image on the top side of the merged image
    merged_image.paste(image1, (0, 0))

    # Paste the second image on the bottom side of the merged image
    merged_image.paste(image2, (0, max_height))
...
merge_images_vertically(image1_path, image2_path, output_path, format='JPEG', quality=90)
